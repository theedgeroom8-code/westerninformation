...
- Password reset: Login → "Forgot password?" → emailed verification code → new
  ...
  → OTP Settings → OTP Length, currently 8). The app's entry screen is
  length-agnostic (one plain field, no digit count in the copy), so changing
  that setting needs no code change (see EMAIL-SETUP.md, Part 3).
